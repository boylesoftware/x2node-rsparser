...
class PolymorphicObjectTypeHandler extends ColumnHandler {
	...
	constructor(colInd, anchorHandler, parentHandler, propDesc, parser) {
		...
		this._typePropName = propDesc.typePropertyName;
		this._nullChecker = parser.valueExtractors['isNull'];
	...
	execute(rowNum, rawVal) {
		...
		if (this._parentHandler.isPropertySet(this._typePropName))
			throw this._parser.invalidData(
...
class ResultSetParser {
	...
	init(markup) {
		...
		const lastColInd = this._parseObjectMarkup(
			0, null, 0, new RootHandler(this), this._topRecordTypeDesc, false);
	 ...
	 * @param {boolean} polymorph <code>true</code> if polymorphic container
	 * pseudo-object.
			...
			if (colInd === 0) {
				...
				if (++colInd < this._numColumns)
					colInd = this._parseObjectMarkup(
						...
						propDesc.nestedProperties, propDesc.isPolymorph());
				...
				if (++colInd < this._numColumns)
					colInd = this._parseObjectMarkup(
						...
						propDesc.nestedProperties, propDesc.isPolymoprh());
				break;
