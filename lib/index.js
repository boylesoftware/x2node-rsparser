...
class PolymorphicObjectTypeHandler {
	...
	constructor(
		colInd, anchorHandler, parentHandler, propDesc, superHandler, type,
		parser) {
	...
	execute(rowNum, rawVal) {
		...
		// register the fact that we got a subtype value
		this._superHandler.gotValue(rowNum, this._colInd);
...
class SingleRefHandler {
	...
	constructor(colInd, parentHandler, propDesc, parser) {
		...
		this._parentHandler = parentHandler;
		this._propName = propDesc.name;
		this._referredRecordType = propDesc.refTargets[0];
...
class SinglePolymorphicRefHandler {
	...
	constructor(colInd, parentHandler, propDesc, superHandler, type, parser) {
	...
	execute(rowNum, rawVal) {
		...
		// skip the column if no reference of the type
		if (referredRecId === null)
			return this._colInd + 1;

		// register the fact that we got a reference value
		this._superHandler.gotValue(rowNum, this._colInd);
...
class SingleRowAnchorHandler {
	...
	execute(rowNum, rawVal) {
		...
		if (nullAnchor)
			return this._colInd + 2; // note: should always be always last
...
class RSParser {
	...
	_parseObjectMarkup(
		...
		do {
			...
			if (fetchRef && (
				!propDesc.isRef() || !propDesc.isScalar() ||
					propDesc.isPolymorph()))
				throw new RSMarkupSyntaxError(
					'Markup column ' + colInd + ': record type ' +
						container.recordTypeName + ' property ' +
						container.nestedPath + propName +
						' is not a scalar non-polymorphic reference and cannot' +
						' be fetched.');
			...
			if (colInd === 0) {
				...
				if (++colInd < this._numColumns)
					colInd = this._parsePolymorphicObjectMarkup(
						colInd, levelPrefix, lastAnchorColInd, parentHandler,
						container, propDesc, superHandler);
				break;
				...
				anchorHandler = new SingleRowAnchorHandler(
					colInd, parentHandler, propDesc, this);
				this._columnHandlers[colInd] = anchorHandler;
				if (++colInd < this._numColumns) {
					this._columnHandlers[colInd] = new ArrayValueHandler(
	...
	_parsePolymorphicObjectMarkup(
		...
		do {
			...
			const handler = new PolymorphicObjectTypeHandler(
				colInd, this._columnHandlers[lastAnchorColInd], parentHandler,
				propDesc, superHandler, type, this);
			this._columnHandlers[colInd] = handler;
			if (++colInd < this._numColumns)
				colInd = this._parseObjectMarkup(
					colInd, levelPrefix, lastAnchorColInd, handler,
		...
		do {
			...
			if (fetchRef) {
				const handler = new SingleFetchedPolymorphicRefHandler(
					colInd, this._columnHandlers[lastAnchorColInd],
					parentHandler, propDesc, superHandler, type, this);
				this._columnHandlers[colInd] = handler;
				if (++colInd < this._numColumns)
					colInd = this._parseObjectMarkup(
						colInd, levelPrefix, lastAnchorColInd, handler,
				...
				this._columnHandlers[colInd] = new SinglePolymorphicRefHandler(
					colInd, parentHandler, propDesc, superHandler, type, this);
				colInd++;
			}
